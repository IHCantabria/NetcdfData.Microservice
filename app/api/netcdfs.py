...
@router.post("/get_netcdf_from_point")
def get_netcdf_from_point(
    product_id : int, longitude: float, latitude: float, start_date = None, end_date = None, variables: list = None 
):
    """_summary_
    ...
    if path != None:
        ds = get_netcdf_point(longitude, latitude, path, start_date, end_date, variables)
...
@router.post("/get_netcdf_from_area")
def get_netcdf_from_area(
    longitude_min: float, longitude_max : float, latitude_min: float, latitude_max : float, product_id : int, start_date = None, end_date = None, variables: list = None
):
    """_summary_
    ...
    path = product[0]["physicalPath"]
    if path != None:
        ds = get_netcdf_area(longitude_min, longitude_max, latitude_min, latitude_max, path, start_date, end_date, variables)
        unique_filename = str(uuid.uuid4())
        ds.to_netcdf("/tmp/{0}.nc".format(unique_filename))
        return FileResponse(
            "/tmp/{0}.nc".format(unique_filename),
...
@router.post("/get_netcdf_from_mask")
def get_netcdf_from_mask(
    filepath_mask : str, product_id : int, row_ID = None, start_date = None, end_date = None, variables: list = None
):
    """_summary_
    ...
    path = product[0]["physicalPath"]
    if path != None:
        ds = get_netcdf_mask(filepath_mask, path, row_ID, start_date, end_date, variables)
        unique_filename = str(uuid.uuid4())
        ds.to_netcdf("/tmp/{0}.nc".format(unique_filename))
        return FileResponse(
            "/tmp/{0}.nc".format(unique_filename),
